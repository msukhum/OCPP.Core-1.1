...
namespace OCPP.Core.Server.Controllers
{
    public partial class AppapiController :  BaseController
    {
        ...
        public async Task<IActionResult> RemoteStartTransaction(string id, string connectorId)
        {
            ...
            {
                Logger.LogWarning("RemoteStartTransaction: Request by non-administrator: {0}", User?.Identity?.Name);
            ...
            Logger.LogTrace("RemoteStartTransaction: Request to unlock chargepoint '{0}'", id);
            if (!string.IsNullOrEmpty(id))
            {
                try
                {
                    using (OCPPCoreContext dbContext = new OCPPCoreContext(this.Config))
                    {
                            ...
                            if (!string.IsNullOrEmpty(serverApiUrl))
                            {
                                try
                                {
                                    using (var httpClient = new HttpClient())
                                    {
                                        ...
                                        uri = new Uri(uri, $"RemoteStartTransaction/{Uri.EscapeUriString(id)}/{Uri.EscapeUriString(connectorId)}");
                                        ...
                                        if (response.StatusCode == HttpStatusCode.OK)
                                        {
                                            ...
                                            if (!string.IsNullOrEmpty(jsonResult))
                                            {
                                                try
                                                {
                                                    ...
                                                    Logger.LogInformation("RemoteStartTransaction: Result of API request is '{0}'", jsonResult);
                                                ...
                                                {
                                                    Logger.LogError(exp, "RemoteStartTransaction: Error in JSON result => {0}", exp.Message);
                                            ...
                                            {
                                                Logger.LogError("RemoteStartTransaction: Result of API request is empty");
                                                httpStatuscode = (int)HttpStatusCode.OK;
                                                resultContent = "An error has occurred.";
                                            }
                                        }
                        ...
                        {
                            Logger.LogWarning("RemoteStartTransaction: Error loading charge point '{0}' from database", id);
                            httpStatuscode = (int)HttpStatusCode.OK;
                ...
                {
                    Logger.LogError(exp, "RemoteStartTransaction: Error loading charge point from database");
                    httpStatuscode = (int)HttpStatusCode.OK;
                    resultContent = "An error has occurred.";
                }
            }
