...
namespace OCPP.Core.Server.Controllers
{
    public partial class OCPPController : BaseController
    {
        ...
        public async Task<IActionResult> SetChargingProfile2ChargePoint(string id, int version)
        {
            ...
            Logger.LogTrace("SetChargingProfile: Request to restart chargepoint '{0}'", id);
            SetChargingProfileRequest request = new SetChargingProfileRequest();
            ...
            try
            {
                ...
                using (var httpClient = new HttpClient())
                {
                    ...
                    {
                        Logger.LogWarning("SetChargingProfile: No API-Key configured!");
                    }
                    ...
                    {
                        Logger.LogError("SetChargingProfile: Result of API  request => httpStatus={0}", response.StatusCode);
                        //httpStatuscode = (int)HttpStatusCode.OK;
            ...
            {
                Logger.LogError(exp, "SetChargingProfile: Error in API request => {0}", exp.Message);
                //httpStatuscode = (int)HttpStatusCode.OK;
                //resultContent = "An error has occurred.";
            }
