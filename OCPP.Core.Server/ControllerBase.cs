...
namespace OCPP.Core.Server
{
    public partial class ControllerBase
    {
        ...
        /// Chargepoint status
        /// </summary>
        public ChargePointStatus ChargePointStatus { get; set; }
        ...
        protected bool UpdateConnectorStatus(int connectorId, string status, DateTimeOffset? statusTime, double? meter, double? chargeRateKW, double? SoC, DateTimeOffset? meterTime)
        {
            try
            {
                using (OCPPCoreContext dbContext = new OCPPCoreContext(Configuration))
                {
                    ...
                    {
                        connectorStatus.LastStatus = status;
                        connectorStatus.LastStatusTime = ((statusTime.HasValue) ? statusTime.Value : DateTimeOffset.Now).DateTime;
                    }

                    if (chargeRateKW.HasValue)
                    {
                        connectorStatus.CurrentChargeKW = chargeRateKW.Value;
                    }

                    if (SoC.HasValue)
                    {
                        connectorStatus.StateOfCharge = SoC.Value;
                    }

                    if (meter.HasValue)
                    {
                        connectorStatus.LastMeter = meter.Value;
                        connectorStatus.LastMeterTime = ((meterTime.HasValue) ? meterTime.Value : DateTimeOffset.Now).DateTime;
                    }
                    ...
                    if (status != "Reserved")
                    {
                        Reservation reservation = dbContext.Reservations.Where(x => x.Status == false && x.ChargePointId == ChargePointStatus.Id && x.ConnectorId == connectorId).FirstOrDefault();
                        if (reservation != null)
                        {
                            reservation.Status = true;
                            reservation.StatusReason = "Reservation Expiry";
            ...
            try
            {
                using (OCPPCoreContext dbContext = new OCPPCoreContext(Configuration))
                {
                    ...
                    if (connectorStatus != null)
                    {
                        ...
                        switch (connectorStatus.LastStatus)
                        {
                            case "Available":
